...
#Market Basket Analysis Product Theme
    ...
    frequent_items2=apriori(df_mba_f, min_support=0.01,use_colnames=True).sort_values('support',ascending=False).reset_index(drop=True)
    frequent_items2['length']=frequent_items2['itemsets'].apply(lambda x: len(x).astype(str).str.contains(fname))
